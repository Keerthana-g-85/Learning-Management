...
export default function AddCourse(){
    ...
    const { Api } = useApi();
    const [course , setCourse] = useState({title : '' , description : '' , instructor_name: user.name , duration:'' ,level:'' , thumbnail:''})
    ...
    function handleDialog(event: React.MouseEvent){
        event.preventDefault()
            ...
            if (!course.title || !course.description || !course.duration  || !course.level || !course.thumbnail ){
                return
            }
            else{
                setOpen(true)
        ...
        <Box  sx={{display:'flex' ,
         ...
         <Dialog open={open} onClose={() => setOpen(false)}>
            ...
            <DialogActions>
                ...
                <Button variant="contained" sx={{bgcolor:"#0ea5e9"}} onClick={handleClick}> Add</Button>
            </DialogActions>
            </Dialog>
        </Box>
        <Snackbar
